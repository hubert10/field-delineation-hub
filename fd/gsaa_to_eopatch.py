...
class DB2Vector(EOTask):
    ...
    def execute(self, eopatch: EOPatch) -> EOPatch:
        ...
        spatial_query = (
            "select * from gsaa where ST_Intersects(ST_GeomFromText((%s), (%s)), geom);"
        )
        ...
        with psycopg2.connect(
            ...
            df = gpd.GeoDataFrame.from_postgis(
                spatial_query, con, geom_col="geom", params=parameters
            )
            df.crs = "EPSG:4326"
            df = (
                df.to_crs(utm_crs)
    ...
    def execute(self, eopatch):
        ...
        for uc in unique_comp:
            if uc != 0:
                ...
                normalised[conn_comp_mask] += distance[conn_comp_mask] / np.max(
                    distance[conn_comp_mask]
                )
...
def get_gsaa_to_eopatch_workflow(config: GsaaToEopatchConfig) -> EOWorkflow:
    ...
    # load patch
    load_task = LoadTask(
        path=f"s3://{config.bucket_name}/{config.eopatches_folder}", config=sh_config
    )

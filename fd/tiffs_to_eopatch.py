...
class AddTimestampsUpdateTime(EOTask):
    ...
    def _get_valid_dates(
        self, tile_name: str, filename: str = "userdata.json"
    ) -> List[datetime]:
        """
...
class CloudMasking(EOTask):
    ...
    def execute(self, eopatch: EOPatch) -> EOPatch:
        ...
        clc = self.s2_cd.get_mask_from_prob(
            eopatch[self.clp_feature].squeeze() / self.max_clp
        )
...
def get_tiffs_to_eopatches_workflow(
    config: TiffsToEopatchConfig, delete_tiffs: bool = False
) -> EOWorkflow:
    ...
    import_mask = (
        ...
        f"Import {config.mask_name}",
    )

    rearrange_bands = (RearrangeBands(), "Swap time and band axis")
    add_timestamps = (
        AddTimestampsUpdateTime(f"s3://{config.bucket_name}/{config.tiffs_folder}"),
        "Load timestamps",
    )
    ...
    remove_bands = (
        RemoveFeature(features={FeatureType.DATA: config.band_names}),
        "Remove bands",
    )

    rename_mask = (
        RenameFeature((FeatureType.MASK, config.mask_name, config.is_data_mask)),
        "Rename is data mask",
    )
    ...
    filenames = [f"{band}.tif" for band in config.band_names] + [
        f"{config.mask_name}.tif",
        f"{config.clp_name}.tif",
        "userdata.json",
    ]
    delete_files = (
        DeleteFiles(path=config.tiffs_folder, filenames=filenames),
        "Delete batch files",
    )

...
def smooth(array: np.ndarray, disk_size: int = 2) -> np.ndarray:
    ...
    smoothed = rank.mean(array, selem=disk(disk_size).astype(np.float32)).astype(
        np.float32
    )
...
class CombineUpsample(EOTask):
    def __init__(
        ...
        feature_extent: Tuple[FeatureType, str],
        feature_boundary: Tuple[FeatureType, str],
        feature_output: Tuple[FeatureType, str],
        scale_factor: int = 2,
        disk_size: int = 2,
    ):
...
class TemporalMerging(EOTask):
    def __init__(
        ...
        feature_merged: Tuple[FeatureType, str],
        start: Union[datetime.date, str],
        end: Union[datetime.date, str],
        percentile: int = 50,
        max_cloud_coverage: float = 0.05,
        valid_data_fraction: float = 1.0,
    ):
    ...
    def execute(self, eopatch: EOPatch) -> EOPatch:
        ...
        for i, ts in enumerate(eopatch.timestamp):
            ...
            if (
                self.start <= ts.date() < self.end
                and cc < self.max_cloud_coverage
                and valid == self.valid_data_fraction
            ):
        ...
        eopatch[self.feature_merged] = np.nan_to_num(
            np.nanpercentile(np.ma.filled(features, np.nan), q=self.percentile, axis=0)
        )
    ...
    def execute(self, eopatch: EOPatch) -> EOPatch:
        ...
        for i, ts in enumerate(eopatch.timestamp):
            if self.start <= ts.date() < self.end:
                masked_data = (
                    eopatch.mask["CLM"][i, ...] | ~eopatch.mask["IS_DATA"][i, ...]
                )
                masked_dilated = (
                    dilation(masked_data.squeeze(), self.disk)[:, :, np.newaxis] == 255
                )
                features.append(
                    np.ma.array(eopatch[self.feature][i, ...], mask=masked_dilated)
                )
...
def get_post_processing_workflow(config: PostProcessConfig) -> LinearWorkflow:
        ...
        merge_extent_tasks = [
            (
                MaskedTemporalMerging(
                    ...
                    dilate_size=config.disk_size_masking,
                ),
    ...
    combine_tasks = [
        (
            CombineUpsample(
                ...
                feature_output=(
                    FeatureType.DATA_TIMELESS,
                    f"PREDICTED_{config.model_version}_{month}",
                ),
                scale_factor=config.scale_factor,
                disk_size=config.disk_size,
            ),
            f"Combine masks for {month}",
        )
    ...
    save_task = (
        SaveTask(
            path=f"s3://{config.bucket_name}/{config.eopatches_folder}",
            features=[
                (FeatureType.DATA_TIMELESS, f"{config.feature_extent[1]}_{month}")
                for month in config.time_intervals
            ]
            + [
...
def get_exec_args(
    workflow: LinearWorkflow, eopatch_list: List[str], config: PostProcessConfig
) -> List[dict]:
        ...
        try:
            ...
            for task_name, task in tasks.items():
                if isinstance(task, ExportToTiffTask):
                    single_exec_dict[task] = dict(
                        filename=f"{name}-{eop.bbox.crs.epsg}.tiff"
                    )

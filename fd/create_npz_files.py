...
def extract_npys(eopatch_path: str, cfg: CreateNpzConfig) -> Tuple:
    ...
    try:
        ...
        y_boundary = np.repeat(
            eop.mask_timeless["BOUNDARY"][np.newaxis, ...], n_timestamps, axis=0
        )
        y_extent = np.repeat(
            eop.mask_timeless["EXTENT"][np.newaxis, ...], n_timestamps, axis=0
        )
        y_distance = np.repeat(
            eop.data_timeless["DISTANCE"][np.newaxis, ...], n_timestamps, axis=0
        )
...
def save_into_chunks(config: CreateNpzConfig, npys_dict: Dict[str, np.ndarray]) -> None:
    eopatches = [
        os.path.basename("_".join(x.split("_")[:-1])) for x in npys_dict["eop_names"]
    ]
    ...
    for idx, i in enumerate(range(0, len(npys_dict["X"]), chunk_size)):
        ...
        np.savez(
            filesystem.openbin(
                os.path.join(config.output_folder, f"{filename}.npz"), "wb"
            ),
        ...
        dfs.append(
            pd.DataFrame(
                dict(
                    chunk=[f"{filename}.npz"]
                    * len(npys_dict["eop_names"][i : i + chunk_size]),

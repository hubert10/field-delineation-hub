...
def stats_per_npz_ts(
    npz_file: str, config: ComputeNormalizationConfig
) -> Dict[str, np.array]:
    filesystem = prepare_filesystem(config)
    data = np.load(
        filesystem.openbin(os.path.join(config.npz_files_folder, npz_file), "rb"),
        allow_pickle=True,
    )
    features = data["X"]
...
def create_per_band_norm_dataframe(
    concatenated_stats: Dict[str, np.array],

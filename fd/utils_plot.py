...
def draw_true_color(
    ...
    def visualize_single_idx(axis, ts):
        axis.imshow(
            np.clip(eopatch.data[feature_name][ts][..., bands] * factor, 0, 1),
...
def draw_outline(o, lw, foreground="black"):
    ...
    """
    o.set_path_effects(
        [patheffects.Stroke(linewidth=lw, foreground=foreground), patheffects.Normal()]
    )
...
def draw_poly(
    ...
    for poly in polys:
        ...
        patch = ax.add_patch(
            patches.Polygon(xy, closed=True, edgecolor=color, fill=False, lw=lw)
        )
...
def draw_bbox(
    ax, eopatch: EOPatch, color: str = "r", lw: int = 2, outline: bool = True
):
    """
...
def draw_mask(
    ax,
    eopatch: EOPatch,
    time_idx: Union[List[int], int, None],
    feature_name: str,
    grid: bool = True,
    vmin: int = 0,
    vmax: int = 1,
    ...
    def _show_single_ts(axis, msk, ts):
        ...
        if grid:
            axis.grid()
        title = (
            f"{feature_name} {eopatch.timestamp[ts]}"
            if ts is not None
            else f"{feature_name}"
        )
...
def draw_vector_timeless(
    ax, eopatch: EOPatch, vector_name: str, color: str = "b", alpha: int = 0.5
):
    """

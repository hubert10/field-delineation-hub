...
def simplify_geometry(
    aoi: Geometry, tolerance: float = 0.004, max_count: int = 1500
) -> Geometry:
    """
...
def plot_batch_splitter(splitter: BatchSplitter):
    ...
    area_series = gpd.GeoSeries(
        [splitter.get_area_shape()], crs=splitter.crs.pyproj_crs()
    )
...
def get_batch_tiles(splitter: BatchSplitter) -> gpd.GeoDataFrame:
    tile_geometries = [
        Geometry(bbox.geometry, bbox.crs) for bbox in splitter.get_bbox_list()
    ]
...
def monitor_batch_job(
    batch: SentinelHubBatch, batch_request: BatchRequest, sleep_time: int = 120
):
    ...
    with tqdm(total=batch_request.tile_count) as progress_bar:
        ...
        while True:
            ...
            new_finished_count = tile_counts.get("PROCESSED", 0) + tile_counts.get(
                "FAILED", 0
            )
            ...
            if finished_count == all_count:
                ...
                if success_count < all_count:
                    ...
                    raise RuntimeError(
                        f"Batch job failed for {all_count - success_count} tiles"
                    )
                break

...
def run_prediction(config: dict):
    ...
    reference_extent = (
        (FeatureType(config["reference_extent"][0]), config["reference_extent"][1])
        if "reference_extent" in config
        else None
    )
    reference_boundary = (
        (FeatureType(config["reference_boundary"][0]), config["reference_boundary"][1])
        if "reference_boundary" in config
        else None
    )
    reference_distance = (
        (FeatureType(config["reference_distance"][0]), config["reference_distance"][1])
        if "reference_distance" in config
        else None
    )
    ...
    prediction_config = PredictionConfig(
        ...
        feature_bands=(
            FeatureType(config["feature_bands"][0]),
            config["feature_bands"][1],
        ),
        feature_extent=(
            FeatureType(config["feature_extent"][0]),
            config["feature_extent"][1],
        ),
        feature_boundary=(
            FeatureType(config["feature_boundary"][0]),
            config["feature_boundary"][1],
        ),
        feature_distance=(
            FeatureType(config["feature_distance"][0]),
            config["feature_distance"][1],
        ),
    ...
    status = process_eopatches(
        run_prediction_on_eopatch,
        eopatches_list,
    ...
    LOGGER.info(
        f'{len(status_df)} total eopatches, {len(status_df[status_df.status!="Success"])} failed'
    )

...
def batch_download(config: dict):
    ...
    output_responses = [
        SentinelHubRequest.output_response(band, MimeType.TIFF)
    ...
    response = get_user_input(
        "Want to start running the batch request? This action will use your PUs: y/n: "
    )
    if response == "n":
        return
    ...
    LOGGER.info(
        f"Status of batch run is: {get_tile_status_counts(batch, batch_request)}"
    )

    response = get_user_input(
        "Did some tiles fail and you want to restart the process? y/n: "
    )
    if response == "y":
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download S2 L1C data using Sentinel Hub Batch Process API.\n"
    )
    parser.add_argument(
        "--config",
        type=str,
        help="Path to config file with data download parameters",
        required=True,
    )

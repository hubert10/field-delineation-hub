...
def _join_overlapping_gdfs(gdf1, gdf2):
    ...
    out = gpd.GeoDataFrame(
        pd.concat([non_overlaps1, non_overlaps2, intersecting]), crs=gdf1.crs
    )
    return out
...
def main(files_to_merge, start_level=0):
    ...
    while len(files_to_merge) > 2:
        ...
        pairs = [
            (level, i, gdf1, gdf2)
            for i, (gdf1, gdf2) in enumerate(
                zip(files_to_merge[0::2], files_to_merge[1::2])
            )
        ]
...
if __name__ == "__main__":
    rows = sorted(
        glob("./contours/FULL/utm32633/*row*gpkg"),

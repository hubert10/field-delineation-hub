...
def rasterise_gsaa(config: dict):
    ...
    gsaa_to_eops_config = GsaaToEopatchConfig(
        ...
        vector_feature=(
            FeatureType(config["vector_feature"][0]),
            config["vector_feature"][1],
        ),
        extent_feature=(
            FeatureType(config["extent_feature"][0]),
            config["extent_feature"][1],
        ),
        boundary_feature=(
            FeatureType(config["boundary_feature"][0]),
            config["boundary_feature"][1],
        ),
        distance_feature=(
            FeatureType(config["distance_feature"][0]),
            config["distance_feature"][1],
        ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Add GSAA reference data to eopatches.\n\n"
        "This script assumes the GSAA vectors are served in a postgis database. Do the following to set-up the database.\n\n"
        "$ docker pull kartoza/postgis\n"
        "$ docker run --name "
        "postgis"
        " -p 25431:5432 -e POSTGRES_USER=niva -e POSTGRES_PASS=n1v4 -e POSTGRES_DBNAME=gisdb -e POSTGRES_MULTIPLE_EXTENSIONS=postgis,hstore -d -t kartoza/postgis\n"

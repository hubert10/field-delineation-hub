...
def merge_zones(config: dict):
    ...
    grid_definition["crs"] = grid_definition["name"].apply(
        lambda name: f"{mgrs_to_utm(name).epsg}"
    )

    utm_geoms = [
        grid_definition[grid_definition["crs"] == crs_name].geometry.unary_union
    ...
    tiled_overlap = grid_definition[
        grid_definition.intersects(overlap)
    ].unary_union.buffer(config["overlap_buffer"])

    zones = gpd.GeoDataFrame(
        geometry=[
            g
            for g in grid_definition[~grid_definition.intersects(tiled_overlap)]
            .buffer(config["zone_buffer"])
            .unary_union
        ],
        crs=grid_definition.crs,
    )
    zones["crs"] = zones.geometry.apply(
        lambda g: grid_definition[grid_definition.intersects(g)]["crs"].unique()[0]
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Merge results for 2 UTM zones into one.\n"
    )
    parser.add_argument(
        "--config",
        type=str,

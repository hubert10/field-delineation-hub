...
def prepare_filesystem(
    config: BaseConfig, sh_config: Optional[SHConfig] = None
) -> S3FS:
    aws_id = config.aws_access_key_id
    aws_secret = config.aws_secret_access_key

    if sh_config and all(
        [sh_config.aws_access_key_id, sh_config.aws_secret_access_key]
    ):
...
def set_sh_config(config: BaseConfig) -> SHConfig:
    ...
    if all(
        key in config.__annotations__.keys()
...
def multiprocess(
    ...
    arguments: Arguments with which the function is called.
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        print("results of multiple processeing")
        # print(process_fun)
        # print(arguments)
        # print(total)
        # print(results)
